...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS ca_users (
        ...
        role TEXT DEFAULT 'viewer', is_active BOOLEAN DEFAULT TRUE, created_at TIMESTAMP DEFAULT NOW()
    )""")
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS collection_logs (
        id SERIAL PRIMARY KEY, property_id INTEGER NOT NULL REFERENCES properties(id),
        month TEXT NOT NULL, total_collection NUMERIC DEFAULT 0, total_invoices NUMERIC DEFAULT 0,
        total_revenue_share NUMERIC DEFAULT 0, notes TEXT DEFAULT '',
        created_by INTEGER NOT NULL REFERENCES ca_users(id), created_at TIMESTAMP DEFAULT NOW()
    )""")
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS user_property_access (
        id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES ca_users(id) ON DELETE CASCADE,
        property_id INTEGER NOT NULL REFERENCES properties(id) ON DELETE CASCADE, UNIQUE(user_id, property_id)
    )""")
    c.execute("""CREATE TABLE IF NOT EXISTS pbi_reports (
        id SERIAL PRIMARY KEY, property_id INTEGER REFERENCES properties(id),
        report_name TEXT NOT NULL, report_type TEXT NOT NULL, pbi_report_id TEXT DEFAULT '',
        pbi_workspace_id TEXT DEFAULT '', embed_url TEXT DEFAULT '', is_active BOOLEAN DEFAULT TRUE
    )""")
...
class PropertyCreate(BaseModel): name: str; location: str = ""; system: str = ""; logo_url: str = ""
...
class ReportCreate(BaseModel):
    ...
    pbi_report_id: str = ""; pbi_workspace_id: str = ""; embed_url: str = ""
...
# ── Property Collection Summary (last 3 months) ───────────────────────────────
...
def fmt_short(n):
    """66000000 → 66M, 1500000 → 1.5M, 500000 → 500K"""
    try:
        v = float(n)
        if v >= 1_000_000: return f"{v/1_000_000:.1f}M".rstrip('0').rstrip('.')+"M" if v/1_000_000 != int(v/1_000_000) else f"{int(v/1_000_000)}M"
        if v >= 1_000: return f"{v/1_000:.1f}K".rstrip('0').rstrip('.')+"K" if v/1_000 != int(v/1_000) else f"{int(v/1_000)}K"
        return f"{v:,.0f}"
    except: return str(n)
...
def build_collection_email(month: str, rows: list, notes: str, portal_url: str) -> str:
    ...
    for r in rows:
        ...
        if coll_rate >= 90:   rate_color="#2CA01C"; rate_bg="#F2FBF0"; rate_border="#B7E5B0"
        elif coll_rate >= 70: rate_color="#B45309"; rate_bg="#FFFBEB"; rate_border="#FDE68A"
        else:                 rate_color="#C80C0F"; rate_bg="#FEF2F2"; rate_border="#FECACA"
        rows_html += f"""
...
def send_collection_email_endpoint(data: CollectionEmailSend, admin=Depends(require_admin)):
    ...
    c.execute("""INSERT INTO email_logs (subject,recipients,property_names,month,status,sent_by)
              ...
              (subject, ", ".join(r["full_name"] for r in recipients),
               ", ".join(props.values()), data.month, f"Sent to {len(sent_to)} recipients", admin["id"]))
    conn.commit(); conn.close()
        ...
        async with httpx.AsyncClient() as client:
            token_resp = await client.post(
                ...
                data={"grant_type": "client_credentials", "client_id": AZURE_CLIENT_ID,
                      "client_secret": AZURE_CLIENT_SECRET, "scope": "https://analysis.windows.net/powerbi/api/.default"})
            ...
            return {"token": embed_data["token"], "tokenId": embed_data["tokenId"],
                    "expiration": embed_data["expiration"], "embedUrl": report_data.get("embedUrl", ""), "reportId": report_id}
